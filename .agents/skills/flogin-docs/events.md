![](data:image/svg+xml;base64,)

Menu

![](data:image/svg+xml;base64,){.feather-menu}

Expand

![](data:image/svg+xml;base64,){.feather-chevron-right}

Light mode

![](data:image/svg+xml;base64,){.feather-sun}

Dark mode

![](data:image/svg+xml;base64,){.icon-tabler-moon}

Auto light/dark, in light mode

![](data:image/svg+xml;base64,){.icon-custom-derived-from-feather-sun-and-tabler-moon}

Auto light/dark, in dark mode

![](data:image/svg+xml;base64,){.icon-custom-derived-from-feather-sun-and-tabler-moon}
![](data:image/svg+xml;base64,){.icon-tabler-pencil-code}
![](data:image/svg+xml;base64,){.icon-tabler-eye-code}

[Skip to content](#furo-main-content){.skip-to-content .muted-link}

:::::::::::::::::::::::::::::::::::::::::::::::: page
:::::::: {.header .mobile-header}
::: header-left
[![](data:image/svg+xml;base64,PHN2Zz48dXNlIGhyZWY9IiNzdmctbWVudSIgLz48L3N2Zz4=)]{.icon}
:::

:::: header-center
[](index.html)

::: brand
flogin 2.0.0a documentation
:::
::::

:::: header-right
::: {.theme-toggle-container .theme-toggle-header}
![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0idGhlbWUtaWNvbi13aGVuLWF1dG8tbGlnaHQiPjx1c2UgaHJlZj0iI3N2Zy1zdW4td2l0aC1tb29uIiAvPjwvc3ZnPg==){.theme-icon-when-auto-light}
![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0idGhlbWUtaWNvbi13aGVuLWF1dG8tZGFyayI+PHVzZSBocmVmPSIjc3ZnLW1vb24td2l0aC1zdW4iIC8+PC9zdmc+){.theme-icon-when-auto-dark}
![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0idGhlbWUtaWNvbi13aGVuLWRhcmsiPjx1c2UgaHJlZj0iI3N2Zy1tb29uIiAvPjwvc3ZnPg==){.theme-icon-when-dark}
![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0idGhlbWUtaWNvbi13aGVuLWxpZ2h0Ij48dXNlIGhyZWY9IiNzdmctc3VuIiAvPjwvc3ZnPg==){.theme-icon-when-light}
:::

[![](data:image/svg+xml;base64,PHN2Zz48dXNlIGhyZWY9IiNzdmctdG9jIiAvPjwvc3ZnPg==)]{.icon}
::::
::::::::

:::::::: {.aside .sidebar-drawer}
::::::: sidebar-container
:::::: sidebar-sticky
[[flogin 2.0.0a
documentation]{.sidebar-brand-text}](index.html){.sidebar-brand}

::: {#searchbox}
:::

:::: sidebar-scroll
::: sidebar-tree
[First Steps]{.caption-text}

- [Introduction](intro.html){.reference .internal}
- [Quickstart](quickstart.html){.reference .internal}

[References]{.caption-text}

- [API Reference](api.html){.reference .internal}
- [Event Reference](#){.current .reference .internal}

[Manuals/Guides]{.caption-text}

- [Search Handlers](search_handlers.html){.reference .internal}
- [Testing your plugin](testing.html){.reference .internal}
- [Installing your plugin](install_plugin.html){.reference .internal}
- [Plugin Settings](settings.html){.reference .internal}
- [Writing Complex Plugins](complex_plugins.html){.reference .internal}
- [Log Override Files](log-override-files.html){.reference .internal}
- [Frequently Asked Questions](faq.html){.reference .internal}

[Library Meta]{.caption-text}

- [Whats New](whats_new.html){.reference .internal}
- [Version Guarantees](version_guarantees.html){.reference .internal}
:::
::::
::::::
:::::::
::::::::

::::::::::::::::::::::::::::::::::: main
::::::::::::::::::::::::::::: content
::::::::::::::::: article-container
[![](data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdib3g9IjAgMCAyNCAyNCI+CiAgICAgICAgICAgIDxwYXRoIGQ9Ik0xMyAyMGgtMlY4bC01LjUgNS41LTEuNDItMS40MkwxMiA0LjE2bDcuOTIgNy45Mi0xLjQyIDEuNDJMMTMgOHYxMnoiIC8+CiAgICAgICAgICA8L3N2Zz4=)
Back to top](#){.back-to-top .muted-link}

::::: content-icon-container
::: view-this-page
[![](data:image/svg+xml;base64,PHN2Zz48dXNlIGhyZWY9IiNzdmctZXllIiAvPjwvc3ZnPg==)
[View this
page]{.visually-hidden}](_sources/events.rst.txt "View this page"){.muted-link}
:::

::: {.theme-toggle-container .theme-toggle-content}
![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0idGhlbWUtaWNvbi13aGVuLWF1dG8tbGlnaHQiPjx1c2UgaHJlZj0iI3N2Zy1zdW4td2l0aC1tb29uIiAvPjwvc3ZnPg==){.theme-icon-when-auto-light}
![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0idGhlbWUtaWNvbi13aGVuLWF1dG8tZGFyayI+PHVzZSBocmVmPSIjc3ZnLW1vb24td2l0aC1zdW4iIC8+PC9zdmc+){.theme-icon-when-auto-dark}
![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0idGhlbWUtaWNvbi13aGVuLWRhcmsiPjx1c2UgaHJlZj0iI3N2Zy1tb29uIiAvPjwvc3ZnPg==){.theme-icon-when-dark}
![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0idGhlbWUtaWNvbi13aGVuLWxpZ2h0Ij48dXNlIGhyZWY9IiNzdmctc3VuIiAvPjwvc3ZnPg==){.theme-icon-when-light}
:::

[![](data:image/svg+xml;base64,PHN2Zz48dXNlIGhyZWY9IiNzdmctdG9jIiAvPjwvc3ZnPg==)]{.icon}
:::::

::::::::::::: {#event-reference .section}
[]{#events}

# Event Reference[¶](#event-reference "Link to this heading"){.headerlink}

You can register an event using the [[`event()`{.xref .py .py-func
.docutils .literal
.notranslate}]{.pre}](api.html#flogin.plugin.Plugin.event "flogin.plugin.Plugin.event"){.reference
.internal} decorator using your [[`Plugin`{.xref .py .py-class .docutils
.literal
.notranslate}]{.pre}](api.html#flogin.plugin.Plugin "flogin.plugin.Plugin"){.reference
.internal} instance. For example:

:::: {.highlight-default .notranslate}
::: highlight
    @plugin.event
    async def on_initialization():
        ...
:::
::::

Alternatively, you can use the [[`register_event()`{.xref .py .py-func
.docutils .literal
.notranslate}]{.pre}](api.html#flogin.plugin.Plugin.register_event "flogin.plugin.Plugin.register_event"){.reference
.internal} method to manually register events. For example:

:::: {.highlight-default .notranslate}
::: highlight
    async def my_event():
        ...

    plugin.register_event(my_event, name="on_initialization")
:::
::::

::: {.admonition .warning}
Warning

All the events must be a
[*coroutine*](https://docs.python.org/3/library/asyncio-task.html#coroutine){.reference
.external}. If they aren't, then you might get unexpected errors. In
order to turn a function into a coroutine they must be
[`async`{.docutils .literal .notranslate}]{.pre}` `{.docutils .literal
.notranslate}[`def`{.docutils .literal .notranslate}]{.pre} functions.
:::

::::: {#api-events .section}
## API Events[¶](#api-events "Link to this heading"){.headerlink}

These events are serialized versions of the raw api events

::: {#on-initialization .section}
[]{#id1}

### on_initialization[¶](#on-initialization "Link to this heading"){.headerlink}

[[async]{.pre} [def]{.pre} [on_initialization()]{.pre}]{.sig-name .descname}

: This function is a
  [*coroutine*](https://docs.python.org/3/library/asyncio-task.html#coroutine){.reference
  .external}.

  This is called when flow sends the [`initialize`{.docutils .literal
  .notranslate}]{.pre} request, which happens when the plugin gets
  started for the first time.
:::

::: {#on-close .section}
[]{#id2}

### on_close[¶](#on-close "Link to this heading"){.headerlink}

[[async]{.pre} [def]{.pre} [on_close()]{.pre}]{.sig-name .descname}

: This function is a
  [*coroutine*](https://docs.python.org/3/library/asyncio-task.html#coroutine){.reference
  .external}.

  This is called when flow attempts to gracefully shut down.

  ::: {.admonition .warning}
  Warning

  Not sending a response in this event WILL cause flow to deadlock. Any
  response will do, even and error response.
  :::
:::
:::::

:::: {#error-handling-events .section}
## Error Handling Events[¶](#error-handling-events "Link to this heading"){.headerlink}

These events are triggered by flogin to handle errors

::: {#on-error .section}
### on_error[¶](#on-error "Link to this heading"){.headerlink}

[[async]{.pre} [def]{.pre} [on_error(event,]{.pre} [error,]{.pre} [\*args,]{.pre} [\*\*kwargs)]{.pre}]{.sig-name .descname}

: This function is a
  [*coroutine*](https://docs.python.org/3/library/asyncio-task.html#coroutine){.reference
  .external}.

  This is called when an error occurs inside of another event.

  Parameters[:]{.colon}

  : - **event** ([[`str`{.xref .py .py-class .docutils .literal
      .notranslate}]{.pre}](https://docs.python.org/3/library/stdtypes.html#str "(in Python v3.14)"){.reference
      .external}) -- The name of the event

    - **error** ([[`Exception`{.xref .py .py-class .docutils .literal
      .notranslate}]{.pre}](https://docs.python.org/3/library/exceptions.html#Exception "(in Python v3.14)"){.reference
      .external}) -- The error that occured

    - **\*args** -- The positional arguments that were passed to the
      event

    - **\*\*kwargs** -- The keyword arguments that were passed to the
      event

  Returns[:]{.colon}

  : Any valid response object for the given event

  Return type[:]{.colon}

  : [[`ErrorResponse`{.xref .py .py-class .docutils .literal
    .notranslate}]{.pre}](api.html#flogin.jsonrpc.responses.ErrorResponse "flogin.jsonrpc.responses.ErrorResponse"){.reference
    .internal} \| [[`QueryResponse`{.xref .py .py-class .docutils
    .literal
    .notranslate}]{.pre}](api.html#flogin.jsonrpc.responses.QueryResponse "flogin.jsonrpc.responses.QueryResponse"){.reference
    .internal} \| [[`ExecuteResponse`{.xref .py .py-class .docutils
    .literal
    .notranslate}]{.pre}](api.html#flogin.jsonrpc.responses.ExecuteResponse "flogin.jsonrpc.responses.ExecuteResponse"){.reference
    .internal}
:::
::::
:::::::::::::
:::::::::::::::::

::::::::: related-pages
[](search_handlers.html){.next-page}

::::: page-info
::: context
Next
:::

::: title
Search Handlers
:::
:::::

![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0iZnVyby1yZWxhdGVkLWljb24iPjx1c2UgaHJlZj0iI3N2Zy1hcnJvdy1yaWdodCIgLz48L3N2Zz4=){.furo-related-icon}
[![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0iZnVyby1yZWxhdGVkLWljb24iPjx1c2UgaHJlZj0iI3N2Zy1hcnJvdy1yaWdodCIgLz48L3N2Zz4=){.furo-related-icon}](api.html){.prev-page}

::::: page-info
::: context
Previous
:::

::: title
API Reference
:::
:::::
:::::::::

:::::: bottom-of-page
:::: left-details
::: copyright
Copyright © 2026, cibere
:::

Made with [Sphinx](https://www.sphinx-doc.org/) and
[\@pradyunsg](https://pradyunsg.me){.muted-link}\'s
[Furo](https://github.com/pradyunsg/furo)
::::

::: right-details
:::
::::::
:::::::::::::::::::::::::::::

::::::: {.aside .toc-drawer}
:::::: {.toc-sticky .toc-scroll}
::: toc-title-container
[ On this page ]{.toc-title}
:::

:::: toc-tree-container
::: toc-tree
- [Event Reference](#){.reference .internal}
  - [API Events](#api-events){.reference .internal}
    - [on_initialization](#on-initialization){.reference .internal}
    - [on_close](#on-close){.reference .internal}
  - [Error Handling Events](#error-handling-events){.reference
    .internal}
    - [on_error](#on-error){.reference .internal}
:::
::::
::::::
:::::::
:::::::::::::::::::::::::::::::::::
::::::::::::::::::::::::::::::::::::::::::::::::
