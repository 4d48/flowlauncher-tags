![](data:image/svg+xml;base64,)

Menu

![](data:image/svg+xml;base64,){.feather-menu}

Expand

![](data:image/svg+xml;base64,){.feather-chevron-right}

Light mode

![](data:image/svg+xml;base64,){.feather-sun}

Dark mode

![](data:image/svg+xml;base64,){.icon-tabler-moon}

Auto light/dark, in light mode

![](data:image/svg+xml;base64,){.icon-custom-derived-from-feather-sun-and-tabler-moon}

Auto light/dark, in dark mode

![](data:image/svg+xml;base64,){.icon-custom-derived-from-feather-sun-and-tabler-moon}
![](data:image/svg+xml;base64,){.icon-tabler-pencil-code}
![](data:image/svg+xml;base64,){.icon-tabler-eye-code}

[Skip to content](#furo-main-content){.skip-to-content .muted-link}

:::::::::::::::::::::::::::::::::::::::::::::::::::::::::::: page
:::::::: {.header .mobile-header}
::: header-left
[![](data:image/svg+xml;base64,PHN2Zz48dXNlIGhyZWY9IiNzdmctbWVudSIgLz48L3N2Zz4=)]{.icon}
:::

:::: header-center
[](index.html)

::: brand
flogin 2.0.0a documentation
:::
::::

:::: header-right
::: {.theme-toggle-container .theme-toggle-header}
![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0idGhlbWUtaWNvbi13aGVuLWF1dG8tbGlnaHQiPjx1c2UgaHJlZj0iI3N2Zy1zdW4td2l0aC1tb29uIiAvPjwvc3ZnPg==){.theme-icon-when-auto-light}
![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0idGhlbWUtaWNvbi13aGVuLWF1dG8tZGFyayI+PHVzZSBocmVmPSIjc3ZnLW1vb24td2l0aC1zdW4iIC8+PC9zdmc+){.theme-icon-when-auto-dark}
![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0idGhlbWUtaWNvbi13aGVuLWRhcmsiPjx1c2UgaHJlZj0iI3N2Zy1tb29uIiAvPjwvc3ZnPg==){.theme-icon-when-dark}
![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0idGhlbWUtaWNvbi13aGVuLWxpZ2h0Ij48dXNlIGhyZWY9IiNzdmctc3VuIiAvPjwvc3ZnPg==){.theme-icon-when-light}
:::

[![](data:image/svg+xml;base64,PHN2Zz48dXNlIGhyZWY9IiNzdmctdG9jIiAvPjwvc3ZnPg==)]{.icon}
::::
::::::::

:::::::: {.aside .sidebar-drawer}
::::::: sidebar-container
:::::: sidebar-sticky
[[flogin 2.0.0a
documentation]{.sidebar-brand-text}](index.html){.sidebar-brand}

::: {#searchbox}
:::

:::: sidebar-scroll
::: sidebar-tree
[First Steps]{.caption-text}

- [Introduction](#){.current .reference .internal}
- [Quickstart](quickstart.html){.reference .internal}

[References]{.caption-text}

- [API Reference](api.html){.reference .internal}
- [Event Reference](events.html){.reference .internal}

[Manuals/Guides]{.caption-text}

- [Search Handlers](search_handlers.html){.reference .internal}
- [Testing your plugin](testing.html){.reference .internal}
- [Installing your plugin](install_plugin.html){.reference .internal}
- [Plugin Settings](settings.html){.reference .internal}
- [Writing Complex Plugins](complex_plugins.html){.reference .internal}
- [Log Override Files](log-override-files.html){.reference .internal}
- [Frequently Asked Questions](faq.html){.reference .internal}

[Library Meta]{.caption-text}

- [Whats New](whats_new.html){.reference .internal}
- [Version Guarantees](version_guarantees.html){.reference .internal}
:::
::::
::::::
:::::::
::::::::

::::::::::::::::::::::::::::::::::::::::::::::: main
::::::::::::::::::::::::::::::::::::::::: content
::::::::::::::::::::::::::::: article-container
[![](data:image/svg+xml;base64,PHN2ZyB4bWxucz0iaHR0cDovL3d3dy53My5vcmcvMjAwMC9zdmciIHZpZXdib3g9IjAgMCAyNCAyNCI+CiAgICAgICAgICAgIDxwYXRoIGQ9Ik0xMyAyMGgtMlY4bC01LjUgNS41LTEuNDItMS40MkwxMiA0LjE2bDcuOTIgNy45Mi0xLjQyIDEuNDJMMTMgOHYxMnoiIC8+CiAgICAgICAgICA8L3N2Zz4=)
Back to top](#){.back-to-top .muted-link}

::::: content-icon-container
::: view-this-page
[![](data:image/svg+xml;base64,PHN2Zz48dXNlIGhyZWY9IiNzdmctZXllIiAvPjwvc3ZnPg==)
[View this
page]{.visually-hidden}](_sources/intro.rst.txt "View this page"){.muted-link}
:::

::: {.theme-toggle-container .theme-toggle-content}
![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0idGhlbWUtaWNvbi13aGVuLWF1dG8tbGlnaHQiPjx1c2UgaHJlZj0iI3N2Zy1zdW4td2l0aC1tb29uIiAvPjwvc3ZnPg==){.theme-icon-when-auto-light}
![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0idGhlbWUtaWNvbi13aGVuLWF1dG8tZGFyayI+PHVzZSBocmVmPSIjc3ZnLW1vb24td2l0aC1zdW4iIC8+PC9zdmc+){.theme-icon-when-auto-dark}
![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0idGhlbWUtaWNvbi13aGVuLWRhcmsiPjx1c2UgaHJlZj0iI3N2Zy1tb29uIiAvPjwvc3ZnPg==){.theme-icon-when-dark}
![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0idGhlbWUtaWNvbi13aGVuLWxpZ2h0Ij48dXNlIGhyZWY9IiNzdmctc3VuIiAvPjwvc3ZnPg==){.theme-icon-when-light}
:::

[![](data:image/svg+xml;base64,PHN2Zz48dXNlIGhyZWY9IiNzdmctdG9jIiAvPjwvc3ZnPg==)]{.icon}
:::::

::::::::::::::::::::::::: {#introduction .section}
[]{#intro}

# Introduction[¶](#introduction "Link to this heading"){.headerlink}

This is an introduction to the flogin library, which aims to make it
easy for python developers to create advanced plugins for Flow Launcher
using it's V2 jsonrpc api.

::: {#prerequisites .section}
## Prerequisites[¶](#prerequisites "Link to this heading"){.headerlink}

flogin requires Python 3.11 or higher. Python 3.11 is recommended as it
matches flow's default embedded Python version.
:::

:::::::::: {#installing .section}
[]{#id1}

## Installing[¶](#installing "Link to this heading"){.headerlink}

::: {.admonition .note}
Note

A [Virtual
Environment](https://docs.python.org/3/library/venv.html){.reference
.external} is recommended to install the library.
:::

To install flogin, do the following:

:::: {.highlight-sh .notranslate}
::: highlight
    pip install -U flogin

    # or with extra dependencies

    pip install -U flogin[extra]
:::
::::

To install the development version, ensure
[git](https://git-scm.com/){.reference .external} is installed, then do
the following:

:::: {.highlight-sh .notranslate}
::: highlight
    pip install git+https://github.com/cibere/flogin
:::
::::

:::: {#extra-dependencies .section}
### Extra Dependencies[¶](#extra-dependencies "Link to this heading"){.headerlink}

::: {.docutils .container}

[[pip]{.pre}]{.sig-name .descname}

: This will install the extra dependencies required by the [[`Pip`{.xref
  .py .py-class .docutils .literal
  .notranslate}]{.pre}](api.html#flogin.pip.Pip "flogin.pip.Pip"){.reference
  .internal} class.
:::
::::
::::::::::

::::::::::::::: {#basic-concepts .section}
## Basic Concepts[¶](#basic-concepts "Link to this heading"){.headerlink}

::: {#callbacks .section}
### Callbacks[¶](#callbacks "Link to this heading"){.headerlink}

With flogin's architecture, you'll primarily work with callbacks. A
callback is a specific function tied to an object that gets triggered
when a corresponding event occurs for that object.
:::

:::::: {#search-handlers .section}
### Search handlers[¶](#search-handlers "Link to this heading"){.headerlink}

For handling search/query requests with flogin, you will use [[search
handlers]{.std
.std-ref}](search_handlers.html#search-handlers){.reference .internal}.
See the [[search handlers section]{.std
.std-ref}](search_handlers.html#search-handlers){.reference .internal}
for an in-depth look at them, but for now, here is a quick version:

You can use the [[`search()`{.xref .py .py-func .docutils .literal
.notranslate}]{.pre}](api.html#flogin.plugin.Plugin.search "flogin.plugin.Plugin.search"){.reference
.internal} decorator to register a search handler, with an async
callback. The callback takes a single argument ([[`Query`{.xref .py
.py-class .docutils .literal
.notranslate}]{.pre}](api.html#flogin.query.Query "flogin.query.Query"){.reference
.internal}), and returns an [[`Result`{.xref .py .py-class .docutils
.literal
.notranslate}]{.pre}](api.html#flogin.jsonrpc.results.Result "flogin.jsonrpc.results.Result"){.reference
.internal} object, a list of [[`Result`{.xref .py .py-class .docutils
.literal
.notranslate}]{.pre}](api.html#flogin.jsonrpc.results.Result "flogin.jsonrpc.results.Result"){.reference
.internal} objects, or anything that will get casted to a string and
converted into an [[`Result`{.xref .py .py-class .docutils .literal
.notranslate}]{.pre}](api.html#flogin.jsonrpc.results.Result "flogin.jsonrpc.results.Result"){.reference
.internal} object.

::: {.admonition .note}
Note

Unlike [[`event()`{.xref .py .py-func .docutils .literal
.notranslate}]{.pre}](api.html#flogin.plugin.Plugin.event "flogin.plugin.Plugin.event"){.reference
.internal}, with [[`search()`{.xref .py .py-func .docutils .literal
.notranslate}]{.pre}](api.html#flogin.plugin.Plugin.search "flogin.plugin.Plugin.search"){.reference
.internal} you must call the decorator, as there are optional arguments
that could be passed.
:::

:::: {.highlight-py .notranslate}
::: highlight
    plugin = Plugin()

    @plugin.search()
    async def my_search_handler(query):
        return "Hello!"
:::
::::
::::::

:::::: {#results .section}
### Results[¶](#results "Link to this heading"){.headerlink}

You can use the [[`Result`{.xref .py .py-class .docutils .literal
.notranslate}]{.pre}](api.html#flogin.jsonrpc.results.Result "flogin.jsonrpc.results.Result"){.reference
.internal} object constructor to pass most options.

::: {.admonition .note}
Note

For handling what happens when the result gets clicked or customizing
the context menu, subclass the object and override the methods. See
[[`Result`{.xref .py .py-class .docutils .literal
.notranslate}]{.pre}](api.html#flogin.jsonrpc.results.Result "flogin.jsonrpc.results.Result"){.reference
.internal} for more info. Alternatively, you can use
[[`create_with_partial()`{.xref .py .py-func .docutils .literal
.notranslate}]{.pre}](api.html#flogin.jsonrpc.results.Result.create_with_partial "flogin.jsonrpc.results.Result.create_with_partial"){.reference
.internal} to create a result with a callback, without subclassing.
:::

:::: {.highlight-py .notranslate}
::: highlight
    plugin = Plugin()

    @plugin.search()
    async def my_search_handler(query):
        return Result(
            title=f"Your text: {query.text}",
            sub="boo",
            copy_text=query.text
        )
:::
::::
::::::

::::: {#events .section}
### Events[¶](#events "Link to this heading"){.headerlink}

At a low level, flogin revolves around a concept called [[events]{.std
.std-ref}](events.html#events){.reference .internal}. An event is
something that you listen for, then respond to. For example, when flow
starts and runs your plugin, it will send a [[on_initialization]{.std
.std-ref}](events.html#on-initialization){.reference .internal} event
that we can listen for.

A quick example code to showcase this:

:::: {.highlight-py .notranslate}
::: highlight
    from flogin import Plugin, Query

    plugin = Plugin()

    @plugin.event
    async def on_initialization():
        # Plugin has started

    plugin.run()
:::
::::
:::::
:::::::::::::::
:::::::::::::::::::::::::
:::::::::::::::::::::::::::::

::::::::: related-pages
[](quickstart.html){.next-page}

::::: page-info
::: context
Next
:::

::: title
Quickstart
:::
:::::

![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0iZnVyby1yZWxhdGVkLWljb24iPjx1c2UgaHJlZj0iI3N2Zy1hcnJvdy1yaWdodCIgLz48L3N2Zz4=){.furo-related-icon}
[![](data:image/svg+xml;base64,PHN2ZyBjbGFzcz0iZnVyby1yZWxhdGVkLWljb24iPjx1c2UgaHJlZj0iI3N2Zy1hcnJvdy1yaWdodCIgLz48L3N2Zz4=){.furo-related-icon}](index.html){.prev-page}

::::: page-info
::: context
Previous
:::

::: title
Home
:::
:::::
:::::::::

:::::: bottom-of-page
:::: left-details
::: copyright
Copyright © 2026, cibere
:::

Made with [Sphinx](https://www.sphinx-doc.org/) and
[\@pradyunsg](https://pradyunsg.me){.muted-link}\'s
[Furo](https://github.com/pradyunsg/furo)
::::

::: right-details
:::
::::::
:::::::::::::::::::::::::::::::::::::::::

::::::: {.aside .toc-drawer}
:::::: {.toc-sticky .toc-scroll}
::: toc-title-container
[ On this page ]{.toc-title}
:::

:::: toc-tree-container
::: toc-tree
- [Introduction](#){.reference .internal}
  - [Prerequisites](#prerequisites){.reference .internal}
  - [Installing](#installing){.reference .internal}
    - [Extra Dependencies](#extra-dependencies){.reference .internal}
  - [Basic Concepts](#basic-concepts){.reference .internal}
    - [Callbacks](#callbacks){.reference .internal}
    - [Search handlers](#search-handlers){.reference .internal}
    - [Results](#results){.reference .internal}
    - [Events](#events){.reference .internal}
:::
::::
::::::
:::::::
:::::::::::::::::::::::::::::::::::::::::::::::
::::::::::::::::::::::::::::::::::::::::::::::::::::::::::::
